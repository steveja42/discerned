...
- **Tier 0 captures the whole CONVERSATION, not just the focused tweet.** An `article`/`full-page` capture on a status page appends each reply as its own card, wrapped in `<div class="tweet-thread">` with each reply in `<div class="tweet-reply">`. `collectTweetReplyArticles()` does the grouping, and its three rules come from measuring the live new-shape DOM with `tests/e2e/tools/x-thread-probe.spec.ts` (`XTHREAD=1`) rather than from the markup's appearance:
  - **The container is derived by climbing from the FOCUSED tweet**, not taken as the lowest common ancestor of every `<article>` on the page. A status page carries articles OUTSIDE the conversation (the sidebar's "Relevant people" card), and including them widens the ancestor to a page-level wrapper whose children are whole regions — at which point the focused tweet and its replies collapse into ONE child and **zero** replies are found. Caught by a unit test, not by reading the code.
  - **Articles NEST, so each container child contributes its OUTERMOST article.** The focused tweet's `<li>` held 2 articles, the inner one being its quoted tweet; a flat `querySelectorAll('article')` emits that quote as though it were a reply.
  - **A reply must carry its own `/handle/status/<id>` link**, which is what excludes non-post cells.
  Replies are capped at `MAX_TWEET_REPLIES` (25) because each card inlines its avatar and photos as base64. `selection` format never collects replies, and the per-reply calls pass `withReplies:false` so a reply can't recurse. Reply media go LAST in `imageUrls` so `thumbnailUrl` stays the focused tweet's own image. In `bodyText` the replies follow a **plain-text** `Replies (N):` line — that field feeds the kind-1 teaser and the library search index, so markdown there would show as literal characters; the cast's horizontal rules come from the `tweet-reply-separator` rule in `html-to-markdown.ts` instead, since the long-form markdown is derived from `bodyHtml`. Guarded by `tests/extraction/tweet-thread.test.ts` and the live `x-thread-visual` spec.
...
| **bsky.app** | `tagBsky` | — | Profile feed or thread with 44px round avatar pin + name/handle row + body + reply/repost/like row per post. Three things were measured here rather than reasoned about (`tools/bsky-layout-probe`, `tools/bsky-gif-probe`): (1) a REPLY nests avatar + name + body in ONE wrapper, so `commonWrapper(avatar, name)` resolved to the whole post and tagging it `dx-header` made the entire reply a byline — a reply rendered as one line reading "rissa @chalissa · 2h that looks immaculate". `dx-header` is now skipped when the wrapper is the post itself or contains the stats row. (2) The byline then still stacked, because `.dx-header:has(img.dx-avatar) > *` sets `max-width: none !important` — a non-important cap is silently ignored, so the name anchor kept measuring the FULL header width and could never fit beside the avatar. Measured whole-clip: 1 of 51 bylines on one line before, 51 of 51 after. (3) A reply's "image" is usually a GIF: a posterless `<video autoplay loop>` from `k.gifs.bsky.app` with **no `poster` and no `src`** (URLs live only in `<source>` children), which every image path missed. | `bsky-visual` (live), `bsky-thread-fixture-visual` (fixture, **pixel baseline** — runs `tagBsky` via `hostOverride`; fixture is a real snapshotted thread with all avatars baked in as data URIs via Node.js-side fetching to bypass CORS), corpus `bsky-thread` + `bsky-profile` |
...
| **apnews.com** (generic) | (none — comment-widget-aware Tier 1 + layout finder) | — | Story headline + byline + hero + body paragraphs; the Viafoura reader-comments thread is dropped (or rides below), never replaces the story. AP pages have no real `<article>` — only `<main class="Page-main">` — but Viafoura renders each comment as `<article class="vf3-comment">`, so with `smartArticleDetection:false` (the sweep default) **`findArticleElement`'s first `article` match was a comment**. Fixed generically at THREE layers, all keyed off `COMMENT_WIDGET_SELECTOR` (Viafoura/Disqus/Coral/OpenWeb/FB/Hyvor/Commento): `findArticleElement` skips comment-widget `<article>`s (→ falls through to `<main>`), `scoreContentBlock` won't pick a comment block, and `markExcluded` drops the widget from a whole-`<main>` capture. **Must be verified LIVE** (offline fixtures + a layout-finder-only fix falsely pass — see [memory: project_comment_widget_layout_finder_fix]). A SECOND, unrelated defect (sweep 2026-09-11, `regressed` on apnews + apnews-tech) buried that story under ~4700px of chrome: AP leads a story with a **video player + "More Videos" rail** of unrelated clips, and its **6-slide photo carousel expanded to all six slides**. Three generic fixes, none per-site: the playlist headings joined BOTH `STRONG_RELATED_RE` and `RELATED_HEADING_RE` (seeds are filtered by the latter, so a heading missing there never reaches the strong branch — that alone left the rail in place); `markExcluded` drops a `position:absolute` slide offset `left:±N00%` when a sibling sits at a different percentage (`isCarouselSlide` — the sibling test is what keeps a lone offset badge/flyout); and the byline pass removes a video **transport-control bar** (≥2 replay/next/mute buttons, or a timecode pair), which otherwise sanitises to a bare glyph+digit strip. | `comment-widget.test.ts` (both `smartArticleDetection` modes, now covering the rail + carousel too) + `apnews-article` sidecar in `article.test.ts` + `apnews-article-fixture-visual` (**pixel baseline**, `AP_FIX=1`); live via the corpus sweep (`SWEEP_ONLY=apnews` → `test-output/corpus-sweep-run/apnews--2-clip.png`) |
