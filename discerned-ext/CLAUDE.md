...
| **facebook.com** | `tagFacebook` | — | Home feed: the topmost visible post as one card — author byline (`<name> is with <name>`) + post text + photo grid + reaction counts. Permalinks (`/photo/`, `/posts/`) scope to the lightbox instead. Composer, stories rail, Like/Comment/Share toolbar and the 88 aria-hidden font-measurement scaffolds dropped. | `facebook-feed-fixture-visual` (fixture, **pixel baseline** — runs `tagFacebook` via `hostOverride`; all images baked to data URIs because fbcdn URLs carry expiring signed tokens and 403 within days). Diagnose the card boundary with `tools/fb-card-probe.spec.ts` (`FBCARD=1`). |
