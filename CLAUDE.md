...
- **Extension ID.** The extension is live on the Chrome Web Store at `https://chromewebstore.google.com/detail/discerned/gpfeknmodijdlehpnkfannklhplmfoma` — store ID **`gpfeknmodijdlehpnkfannklhplmfoma`**, now in `ALLOWED_ORIGINS` in `discerned-web/netlify/functions/feedback.mts`. The `key` field was **removed from `manifest.json`** for submission — the store assigns its own ID and signs with its own key, and an uploaded package carrying a `key` is rejected. The old pinned ID was `egocpdhpffaddnhjimclgabdhpbjbhod` (private `.pem` outside the repo at `~/.discerned-keys/discerned-ext.pem`); keep the `.pem` so the side-loaded zip build can be re-pinned if ever needed. **Consequence:** an unpacked/side-loaded install gets a different ID from the store build, so its IndexedDB (`discerned`) is a *separate store*. Existing side-load users do not carry clips over to the store build — that migration is not automatic.
...
```powershell
# 1. Start it. Preflights, backs up the previous run, captures in the BACKGROUND,
#    and starts a SECOND hidden process that slices each domain's clip+cast top
#    band the moment it lands (batched — see below) and logs it to
#    test-output/sweep-slices.log.
...
- **Capture and review overlap.** The sweep writes each domain's three PNGs + `score.json` the moment that domain finishes, so reviewing starts within minutes instead of after the whole run. `review-queue.mjs` lists only what is captured-but-unreviewed (worst composite first) and prints image paths — never image bytes — so the review loop stays token-cheap. A verdict is *stale* when its PNG is newer than its `reviewedAt` stamp, which is how a re-captured domain gets re-reviewed instead of keeping a verdict describing the old image.
...
`SWEEP_UNATTENDED=1` is set by the script: a full run has dozens of gated domains, and stopping for clicks is what turns two hours into a day. Domains that genuinely need a human click are recovered afterwards via `--project=corpus-sweep-manual`. Expect a residue of hard-walled domains every run — mark those `blocked` rather than chasing them, and see [memory: project_sweep_wall_is_cumulative_not_per_site] before treating a block as a property of the site.
